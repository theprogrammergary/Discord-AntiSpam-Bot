...
async def notifiy_new_payout(member, channel) -> None:
    ...
    embed_description: str = (
        f"{member.mention} just received a payout from Apex Trader Funding!"
    )
    ...
    if config.GG_ICON:
        embed.set_footer(
            text=(
                f'to {member.display_name}...use code "TBM" at APEX for the best available discount.'
            ),
    ...
    if config.GG_ICON:
        embed.set_footer(
            text=(
                ...
                '...use code "TBM" at APEX for the best available discount.'
            ),
            icon_url=config.GG_ICON,
        )
